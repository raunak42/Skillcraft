...
export async function POST(req: Request): Promise<Response> {
    try {
        ...
        if (!sessionDataHeader) {
            return Response.json({ message: "sessionDataHeader not found" });
        }
        ...
        const body: CourseAttributes = await req.json();//giving it a type is important or else the prisma call below won't recognize the data while using a spred operator.
        const course = body
        ...
        return Response.json({ message: "course created successfully" }, { status: 200 })

...
const unprotectedRoutes = ["/api/getCourses", "/api/getFeaturedCourses", "/api/getTopCourses", "/api/search", "/api/checkoutSession"] //for ssr //could have gone with the matcher approach for ssr routes but mathcer blocks the entire middleware for the path that is not included, that means session verification would have stopped but on top of that csrf prevention would also have stopped.
const noMiddlewareRoutes = ["/api/signup", "/api/login", "/api/checkoutSession", "/api/webhook"]
...
export async function middleware(request: NextRequest): Promise<Response | undefined> {
	...
	if (!unprotectedRoute) {
		...
		if (!session) {
			return NextResponse.json({ message: "Sign in firstttt." }, {
				status: 403
			})
		}
		// const response = NextResponse.next();
